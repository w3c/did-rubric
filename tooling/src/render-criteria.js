    ...
    return `
        <section class="criteria">
           ...
           <span>v${criteria.version}</span>
            ...
            `<section>
                <h5>Responses</h5>
                <ul class="responses" style="list-style-type: none; padding: 0;">
                ${criteria.response.possibleResponses.map(response => `
                    <li><span class="response-label">${response.label}:</span> ${response.meaning || ''}</li>
                `).join('')}
                </ul>
            </section>` : ''}
            ...
            <tbody>
                ${criteria.exampleAssessments.map(assessment => `
                <tr>
                    ${criteria.assessmentTemplate && criteria.assessmentTemplate.columns ? criteria.assessmentTemplate.columns.map(column => {
